...
export class GetLessonAvailableInfoQueryHandler
  implements GetLessonAvailableInfoInboundPort
{
  constructor(
  ...
  async execute(
    query: GetLessonAvailableInfoInboundPortInputDto,
  ): Promise<string[]> {
    ...
    return this.generateDateTimeSlots(lessonList, query.lessonTime);
  }
  generateDateTimeSlots(lessons: LessonEntity[], lessonLength: string) {
    const dates = [];
    ...
    for (let i = 0; i < 7; i++) {
      ...
      for (let j = 0; j < (23 - 7 + 1) * (60 / Number(lessonLength)); j++) {
        ...
        let strNewDate = newDate
          .toISOString()
          .replace('T', ' ')
          .substring(0, 19);

        if (!this.isTimeSlotOccupied(strNewDate, lessons)) {
  ...
  isTimeSlotOccupied(timeSlotStr: string, lessons: LessonEntity[]) {
